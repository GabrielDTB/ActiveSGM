...
class ForwardWarping(nn.Module):
    ...
    def create_rgbdm(
        ...
        depths = torch.stack(
            [rgbdm00[:, :, 3], rgbdm01[:, :, 3], rgbdm10[:, :, 3], rgbdm11[:, :, 3]]
        )
        reshaped_rgbdm = torch.stack(
            [
                rgbdm00.reshape(-1, channel),
                rgbdm01.reshape(-1, channel),
                rgbdm10.reshape(-1, channel),
                rgbdm11.reshape(-1, channel),
            ]
        )  # 4,H*W,C
        ...
        for i in range(channel):
            selected_input = torch.gather(
                reshaped_rgbdm[:, i], 1, z_buffer_idx.view(-1, 1)
            )
    ...
    def forward_warping(
        ...
        input_dim = 6
        forward_input = forward_input.reshape(N, input_dim, -1).permute(
            0, 2, 1
        )  # N, M, D
    ...
    def forward(
        ...
        ### forward warping ###
        nv_img, nv_depth, nv_mask = self.forward_warping(
            img, transformed_distance, nv_grid
        )
        ...
        ### compute valid mask ###
        invalid_depth_mask = (nv_depth > 1e6) * 1.0  # maximum is 1e8
        nv_mask = ((nv_mask + invalid_depth_mask) > 0) * 1.0
...
if __name__ == "__main__":
    ...
    forward_warp = ForwardWarping(out_hw=out_hw, device=device, K=K)
    ...
    print("==> ForwardWarp: {}ms".format((time() - start) * 1000))
    nv_img_np = nv_img[0].permute(1, 2, 0).detach().cpu().numpy().astype(np.uint8)
    nv_mask_np = np.clip(nv_mask[0, 0].detach().cpu().numpy() * 255, 0, 255).astype(
        np.uint8
    )
    ...
    if bs > 0:
        nv_img_inpaint = cv2.inpaint(
            nv_img_np[bs:-bs, bs:-bs], nv_mask_np[bs:-bs, bs:-bs], 3, inpaint_method
        )
        start = time()
        nv_depth_inpaint = cv2.inpaint(
            nv_depth_np[bs:-bs, bs:-bs], nv_mask_np[bs:-bs, bs:-bs], 3, inpaint_method
        )
        print("==> Inpaint: {}ms".format((time() - start) * 1000))
    else:
