...
def save_output_video(vis_path, video_path, H, W, output, near_depth, far_depth):
    ...
    with media.VideoWriter(video_file, **video_kwargs, input_format="rgb") as writer:
        for output_view in tqdm(output, desc=f"Rendering video"):
            ...
            depth = (
                np.tile(output_view["depth"].reshape(H, W, 1), (1, 1, 3)) - near_depth
            ) / (far_depth - near_depth)
            image_frame = (np.clip(np.nan_to_num(image), 0.0, 1.0) * 255.0).astype(
                np.uint8
            )
            normal_frame = (np.clip(np.nan_to_num(normal), 0.0, 1.0) * 255.0).astype(
                np.uint8
            )
            depth_frame = (np.clip(np.nan_to_num(depth), 0.0, 1.0) * 255.0).astype(
                np.uint8
            )
