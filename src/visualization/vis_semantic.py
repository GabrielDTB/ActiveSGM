...
def save_semantic_ply(
    params, variables, ckpt_output_dir, time_idx, n_cls=41, colormap=None, mask=None
):
    ...
    sem_rgbs = apply_colormap(class_ids, sem_colormap) / 255.0
...
def c2e(cubemap, h, w, mode="bilinear", cube_format="dice"):
    ...
    equirec = np.stack(
        [
            py360convert.utils.sample_cubefaces(
...
def render_view(
    ...
    depth_rendervar = transformed_params2depthplussilhouette(
        final_params, first_frame_w2c.cuda(), transformed_gaussians
    )
    ...
    (
        ...
        _,
        _,
    ) = Renderer(raster_settings=curr_data["cam"])(**depth_rendervar)
    ...
    seman_rendervar = transformed_params2semrendervar_sparse(
        final_params, transformed_gaussians, seen
    )
    sparse_cam = set_camera_sparse(
        cam=curr_data["cam"], cls_ids=final_variables["seman_cls_ids"]
    )
    ...
    if save_frames:
        ...
        plot_dict["gt_rgb"] = (
            (255 * curr_data["im"]).cpu().permute(1, 2, 0).numpy().astype(np.uint8)
        )
    ...
    for k, v in params.items():
        ...
        if not isinstance(v, torch.Tensor):
            params[k] = torch.nn.Parameter(
                torch.tensor(v).cuda().float().contiguous().requires_grad_(False)
            )
        else:
            params[k] = torch.nn.Parameter(
                v.cuda().float().contiguous().requires_grad_(False)
            )
    return params
...
if __name__ == "__main__":
    ...
    if main_cfg.slam["method"] == "semsplatam":
        ...
        ply_savepath = os.path.join(eval_dir, ply_name)
        sem_opacities = (
            save_params["logit_opacities"].detach().cpu().contiguous().numpy()
        )
