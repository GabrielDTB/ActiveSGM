...
def initialize_params_with_seman(
    init_pt_cld, num_frames, mean3_sq_dist, gaussian_distribution, n_cls=16
):
    ...
    params = {
        ...
        "semantic_logits": torch.zeros(
            (num_pts, n_cls), dtype=torch.float, device="cuda"
        ),
...
def add_new_gaussians_with_seman(
    ...
    if torch.sum(non_presence_mask) > 0:
        ...
        new_logit_opacities = torch.zeros(
            (num_new_pts, 1), dtype=torch.float, device="cuda"
        )
        if gaussian_distribution == "isotropic":
            new_log_scales = torch.tile(
        ...
        new_params = {
            ...
            "rgb_colors": new_rgb,
            "semantic_logits": torch.zeros(
                (num_new_pts, n_cls), dtype=torch.float, device="cuda"
            ),
        ...
        for k, v in new_params.items():
            if not isinstance(v, torch.Tensor):
                v = torch.tensor(v)
            new_params[k] = torch.nn.Parameter(
                v.cuda().float().contiguous().requires_grad_(True)
            )
        ...
        # Update variables
        new_cls_ids = (
            torch.arange(n_cls, device="cuda").unsqueeze(0).expand(num_new_pts, -1)
        )
...
def densify(params, variables, optimizer, iter, densify_dict):
    ...
    if iter <= densify_dict["stop_after"]:
        ...
        if (iter >= densify_dict["start_after"]) and (
            ...
            if "seman_cls_ids" in variables:
                variables["seman_cls_ids"] = torch.cat(
                    (variables["seman_cls_ids"], variables["seman_cls_ids"][to_clone]),
                    dim=0,
                )
            num_pts = params["means3D"].shape[0]
