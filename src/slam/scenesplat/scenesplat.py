...
class SceneSplatam(SplatamOurs):
    def __init__(
        ...
        self.backbone = load_scenesplat_backbone(
            scenesplat_ckpt, pointcept_path, self.device
        )
    ...
    def run_scenesplat_on_map(self):
        ...
        feat_16 = run_scenesplat(
            ...
            self.autoencoder,
            self.params,
            grid_size=self.scenesplat_grid_size,
            device=self.device,
        )
        ...
        self.variables["seman_cls_ids"] = (
            torch.arange(self.n_cls, device=self.device)
            .unsqueeze(0)
            .expand(N, -1)
            .contiguous()
        )
            ...
            with torch.no_grad():
                ...
                if PRINT_INFO:
                    ...
                    if self.slam_cfg.use_global_keyframe:
                        global_kf_times = [
                            t
                            for t in self.global_keyframe_time_indices
                            if t != time_idx
                        ]
                        print(
    ...
    def print_and_save_result(
        ...
        tracking_iter_time_avg = (
            self.tracking_iter_time_sum / self.tracking_iter_time_count
        )
        tracking_frame_time_avg = (
            self.tracking_frame_time_sum / self.tracking_frame_time_count
        )
        mapping_iter_time_avg = (
            self.mapping_iter_time_sum / self.mapping_iter_time_count
        )
        mapping_frame_time_avg = (
            self.mapping_frame_time_sum / self.mapping_frame_time_count
        )
    ...
    def load_params_by_step(self, step=1100, stage="final"):
        ...
        if step == 0:
            ...
            ckpt_path = os.path.join(
                config["workdir"], config["run_name"], f"params{step}.npz"
            )
