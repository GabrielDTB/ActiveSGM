...
class SplatamOurs(SlamModel):
    def __init__(
        ...
        ### Create directories ###
        self.results_dir = os.path.join(config["workdir"], config["run_name"])
        ...
        if not self.config["load_checkpoint"]:
            ...
            shutil.copy(
                self.slam_cfg.room_cfg, os.path.join(self.results_dir, "config.py")
            )
    ...
    def load_splatam_dataset_config(self):
        ...
        if self.seperate_tracking_res:
            ...
            self.tracking_color = (
                tracking_color.permute(2, 0, 1) / 255
            )  # (H, W, C) -> (C, H, W)
