...
class SDFNetNaruto(SDFNet):
    def __init__(
        self, config, input_ch=3, geo_feat_dim=15, hidden_dim=64, num_layers=2
    ):
        ...
        if tcnn_network:
            ...
            for l in range(self.num_layers):
                ...
                if l == self.num_layers - 1:
                    out_dim = (
                        1 + self.geo_feat_dim + uncert
                    )  # 1 sigma + 15 SH features for color + optional uncertainty
                else:
...
class ColorSDFNet_v2_Naruto(nn.Module):
    ...
    def __init__(self, config, input_ch=3, input_ch_pos=12):
        ...
        self.config = config
        self.pred_uncert = (
            config["decoder"]["pred_uncert"] or config["decoder"]["uncert_grid"]
        )
    ...
    def forward(self, embed, embed_pos):
        ...
        if self.pred_uncert:
            ...
            sdf = torch.cat(
                [sdf, uncert], -1
            )  # put uncertainty to last channel of the sdf.
