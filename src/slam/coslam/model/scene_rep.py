...
from third_parties.coslam.model.utils import (
    sample_pdf,
    get_sdf_loss,
    mse2psnr,
    compute_loss,
...
class JointEncodingNaruto(JointEncoding):
    ...
    def get_uncert_grid(self, voxel_size):
        Nx = (
            round(
                (self.bounding_box[0, 1] - self.bounding_box[0, 0]).item() / voxel_size
                + 0.0005
            )
            + 1
        )
        Ny = (
            round(
                (self.bounding_box[1, 1] - self.bounding_box[1, 0]).item() / voxel_size
                + 0.0005
            )
            + 1
        )
        Nz = (
            round(
                (self.bounding_box[2, 1] - self.bounding_box[2, 0]).item() / voxel_size
                + 0.0005
            )
            + 1
        )
        # Uncertainty initialize to 3
        self.uncert_grid = torch.nn.parameter.Parameter(
            torch.ones([Nx, Ny, Nz], device="cuda").float() * 3
        )
    ...
    def calc_embedding(self, inputs):
        ...
        if self.config["decoder"]["uncert_grid"]:
            ...
            uncert = torch.nn.functional.grid_sample(
                self.uncert_grid[None, None, ...], grid, align_corners=False
            )
    ...
    def raw2outputs(self, raw, z_vals, white_bkgd=False):
        ...
        if (
            ...
            uncert = (
                self.act_uncertainty(raw[..., 4]) + 0.01
            )  # 0.01 is the min uncertainty # (HY-000): uncertainty grid/net source
    ...
    def query_sdf(
        self, query_points, return_geo=False, embed=False, return_uncert=False
    ):
        """
        ...
        if embed:
            embedded = self.embed_fn(
                inputs_flat
            )  # without the possible uncertainty grid embedding
            return torch.reshape(
                embedded, list(query_points.shape[:-1]) + [embedded.shape[-1]]
            )
        embedded = self.calc_embedding(
            inputs_flat
        )  # with the possible uncertainty grid embedding
    ...
    def forward(self, rays_o, rays_d, target_rgb, target_d, global_step=0):
        ...
        if "uncert_map" in rend_dict:
            ...
            uncert_loss = torch.mean(
                (1 / (2 * (uncert_map + 1e-9).unsqueeze(-1))) * ((x - y) ** 2)
            ) + 0.5 * torch.mean(torch.log(uncert_map + 1e-9))
