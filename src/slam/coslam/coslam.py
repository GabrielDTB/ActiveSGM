...
class CoSLAMNaruto(SlamModel, CoSLAM):
    def __init__(self, main_cfg: mmengine.Config, info_printer: InfoPrinter) -> None:
        ...
        self.keyframeDatabase = self.create_kf_database(
            config
        )  ### TODO: rm the requirement for ReplicaSLAM data ###
    ...
    def override_config(self, cfg: Dict) -> Dict:
        ...
        ### visualization ###
        cfg["mesh"]["vis"] = self.main_cfg.visualizer.get(
            "mesh_vis_freq", cfg["mesh"]["vis"]
        )
    ...
    def get_loss_from_ret(
        self, ret, rgb=True, sdf=True, depth=True, fs=True, uncert=True, smooth=False
    ):
        ...
        if uncert and (
            self.config["decoder"]["pred_uncert"]
            or self.config["decoder"]["uncert_grid"]
        ):
    ...
    def first_frame_mapping(self, batch, n_iters=100):
        ...
        """
        self.info_printer("First frame mapping...", self.step, self.__class__.__name__)
        ...
        for i in range(n_iters):
            ...
            indice = self.select_samples(
                self.dataset.H, self.dataset.W, self.config["mapping"]["sample"]
            )

            # indice_h, indice_w = indice % (self.dataset.H), indice // (self.dataset.H)
            indice_h, indice_w = (
                indice % (self.dataset.H),
                torch.div(indice, (self.dataset.H), rounding_mode="trunc"),
            )
            rays_d_cam = (
            ...
            rays_o = c2w[None, :3, -1].repeat(
                self.config["mapping"]["sample"], 1
            )  # camera translation (vector to origin)
            rays_d = torch.sum(
                rays_d_cam[..., None, :] * c2w[:3, :3], -1
            )  # ray direction vector in world coords
        ...
        self.info_printer(
            "First frame mapping done", self.step, self.__class__.__name__
        )
        return ret, loss
    ...
    def init_uncert_grid_optim(self, voxel_size=0.1):
        ...
        self.uncert_optim = torch.optim.Adam(
            params=[self.model.get_uncert_grid(voxel_size)], lr=1
        )
    ...
    def global_BA(self, batch, cur_frame_id):
        ...
        if (
            len(self.keyframeDatabase.frame_ids) < 2
            or self.config["tracking"]["disable"]
        ):
        ...
        for i in range(self.config["mapping"]["iters"]):
            ...
            ##################################################
            num_rays_to_sample_cur = max(
                sample_num // len(self.keyframeDatabase.frame_ids), min_pixels_cur
            )
            if not self.config["mapping"]["filter_depth"]:
                idx_cur = random.sample(
                    range(0, self.dataset.H * self.dataset.W), num_rays_to_sample_cur
                )
            else:
                cur_valid_depth = (current_rays[..., -1] > 0.0) & (
                    current_rays[..., -1] <= self.config["cam"]["depth_trunc"]
                )
                cur_rays_valid = current_rays[cur_valid_depth, :]  # [n_valid, 7]
                cur_num_valid = len(cur_rays_valid)
                idx_cur = random.sample(
                    range(0, cur_num_valid), min(cur_num_valid, num_rays_to_sample_cur)
                )
            ...
            if self.config["mapping"]["active_ray"]:
                rays_o, rays_d, target_s, target_d = (
                    self.active_ray_sampler.sample_rays(
                        rays_o,
                        rays_d,
                        target_s,
                        target_d,
            ...
            if (
                ...
                and (i + 1) % self.config["mapping"]["pose_accum_step"] == 0
            ):
                pose_optimizer.step()
    ...
    def save_mesh(
        ...
        mesh_name = (
            f"mesh_{i:04}{suffix}.ply"
            if i is not None
            else f"mesh_{self.step:04}{suffix}.ply"
        )
    ...
    def save_uncert_mesh(
        ...
        ### create mesh dir and get mesh_savepath ###
        if mesh_savedir is None:
            mesh_savedir = os.path.join(
                self.main_cfg.dirs.result_dir, "coslam", "uncert_mesh"
            )
        os.makedirs(mesh_savedir, exist_ok=True)
        mesh_name = (
            f"mesh_{i:04}{suffix}.ply"
    ...
    def save_ckpt(self, i: int = None, suffix: str = "") -> None:
        """save checkpoint
        ...
        ckpt_savedir = os.path.join(
            self.main_cfg.dirs.result_dir, "coslam", "checkpoint"
        )
        os.makedirs(ckpt_savedir, exist_ok=True)
        ckpt_name = (
            f"ckpt_{i:04}{suffix}.pt"
            if i is not None
            else f"ckpt_{self.step:04}{suffix}.pt"
        )
    ...
    def predict_sdf(self, points: torch.Tensor) -> torch.Tensor:
        ...
        """
        sdf = query_point_sdf(
            self.model.query_sdf, points, self.config, self.bounding_box
        )
        return sdf

    def online_recon_step(
        self, i: int, color: torch.Tensor, depth: torch.Tensor, c2w: torch.Tensor
    ) -> List:
        ...
        ### save mesh ###
        if (
            i % self.config["mesh"]["vis"] == 0
        ):  # self.config['mesh']['vis']: visualization frequency. Now: 500
            self.info_printer(
                "Extract and save mesh", self.step, self.__class__.__name__
            )
        ...
        if i == 0:
            ...
            self.info_printer(
                "Get map volumes (UncertaintyVolume, SDFVolume)",
                self.step,
                self.__class__.__name__,
            )
            uncert_sdf_vols = get_map_volumes(
                self.model.query_sdf,
                self.bounding_box,
        ...
        ##################################################
            ...
            if self.config["tracking"]["disable"]:
                ...
                self.info_printer(
                    "Running tracking", self.step, self.__class__.__name__
                )
            ...
            if i % self.config["mapping"]["map_every"] == 0:
                self.info_printer(
                    "Running global BA", self.step, self.__class__.__name__
                )
            ...
            if i % self.config["mapping"]["keyframe_every"] == 0:
                self.info_printer(
                    f"Add keyframe-{self.step} to KeyframeDatabase",
                    self.step,
                    self.__class__.__name__,
                )
                self.keyframeDatabase.add_keyframe(
