...
class SGSSLAMOurs(SplatamOurs):
    def __init__(
        ...
        class_info_file = (
            f"./configs/Replica/{main_cfg.general.scene}/class_info_file.json"
        )
    ...
    def semantic_annotation(self, input_img: torch.Tensor):
        ...
        class_ids_from_oneformer, _ = oneformer_segmentation(
            input_img,
        ...
        class_colors_from_oneformer = (
            apply_colormap(class_ids_from_oneformer.numpy(), self.colormap) / 255.0
        )
    ...
    def load_checkpoint(self):
        ...
        if checkpoint_time_idx == 0:
            ckpt_path = os.path.join(
                config["workdir"], config["run_name"], f"params.npz"
            )
        else:
            ckpt_path = os.path.join(
                config["workdir"],
        ...
        for k in params:
            if k not in params_opt_exclude:
                params[k] = (
                    torch.tensor(params[k]).to(device).float().requires_grad_(True)
                )
            else:
        ...
        variables["means2D_gradient_accum"] = (
            torch.zeros(params["means3D"].shape[0]).cuda().float()
        )
    ...
    def update_gs_map(
        ...
        curr_data = {
            ...
            "im": color,
            "depth": depth,
            "semantic_id": seman_id,
            "semantic_color": seman_color.to(color.dtype),
            "id": iter_time_idx,
        ...
        if seperate_tracking_res:
            ...
            tracking_seman_id = F.interpolate(
                seman_id.unsqueeze(0).float(), (tracking_h, tracking_w), mode="nearest"
            )[0].long()
            tracking_seman_color = F.interpolate(
                seman_color.unsqueeze(0), (tracking_h, tracking_w), mode="bilinear"
            )[0]
            ...
            tracking_curr_data = {
                ...
                "semantic_id": tracking_seman_id,
                "semantic_color": tracking_seman_color.to(tracking_color.dtype),
                "id": iter_time_idx,
            ...
            while True:
                ...
                with torch.no_grad():
                    ...
                    if config["report_iter_progress"]:
                        color_map = torch.from_numpy(self.colormap.copy() / 255.0).to(
                            self.device
                        )
                        if config["use_wandb"]:
                            report_progress(
        ...
        if (
            ...
            if config["mapping"]["add_new_gaussians"] and time_idx > 0:
                ...
                if seperate_densification_res:
                    ...
                    densify_seman_id = F.interpolate(
                        seman_id.unsqueeze(0).float(),
                        (densify_h, densify_w),
                        mode="nearest",
                    )[0].long()
                    densify_seman_color = F.interpolate(
                        seman_color.unsqueeze(0),
                        (densify_h, densify_w),
                        mode="bilinear",
                    )[0]
                    ...
                    densify_curr_data = {
                        ...
                        "depth": densify_depth,
                        "id": time_idx,
                        "semantic_id": densify_seman_id,
                        "semantic_color": densify_seman_color.to(densify_color.dtype),
            ...
            for iter in range(num_iters_mapping):
                ...
                if only_use_global_keyframe or (
                    ...
                    if len(self.global_keyframe_indices) == 1:
                        ...
                        iter_seman_color = keyframe_list[selected_rand_keyframe_idx][
                            "semantic_color"
                        ]
                        iter_seman_id = keyframe_list[selected_rand_keyframe_idx][
                            "semantic_id"
                        ]
                else:
                    rand_idx = np.random.randint(0, len(selected_keyframes))
                ...
                iter_data = {
                    ...
                    "depth": iter_depth,
                    "id": iter_time_idx,
                    "semantic_color": iter_seman_color.to(iter_color.dtype),
                    "semantic_id": iter_seman_id,
                    "intrinsics": intrinsics,
                    "w2c": first_frame_w2c,
    ...
    def print_and_save_result(
        ...
        if is_prune_gaussians:
            optimizer = initialize_optimizer(
                params, params_opt_exclude, config["mapping"]["lrs"], tracking=False
            )
            params, variables = prune_gaussians(
                params,
                params_opt_exclude,
