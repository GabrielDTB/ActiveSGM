...
def get_pointcloud(
    ...
    x_grid, y_grid = torch.meshgrid(
        torch.arange(width).to(device).float(),
    ...
    if load_semantics:
        semantic_id = torch.permute(semantic_id, (1, 2, 0)).reshape(
            -1, 1
        )  # (1, H, W) -> (H, W, 1) -> (H * W, 1)
        semantic_color = torch.permute(semantic_color, (1, 2, 0)).reshape(
            -1, 3
        )  # (3, H, W) -> (H, W, 3) -> (H * W, 3)
...
def initialize_params(
    init_pt_cld, num_frames, mean3_sq_dist, device, load_semantics=False
):
        ...
        if k not in params_opt_exclude:
            ...
            if not isinstance(v, torch.Tensor):
                params[k] = torch.nn.Parameter(
                    torch.tensor(v).to(device).float().contiguous().requires_grad_(True)
                )
            else:
                params[k] = torch.nn.Parameter(
                    v.to(device).float().contiguous().requires_grad_(True)
                )

    variables = {
        "max_2D_radius": torch.zeros(params["means3D"].shape[0]).to(device).float(),
...
def initialize_first_timestep(
    ...
    if densify_dataset is not None:
        ...
        if (semantic_color.shape[1] != H) or (semantic_color.shape[2] != W):
            semantic_color = F.interpolate(
                semantic_color.unsqueeze(0), (H, W), mode="bilinear"
            )[0]
            semantic_id = F.interpolate(
                semantic_id.unsqueeze(0).float(), (H, W), mode="nearest"
            )[0].long()
    ...
    params, variables, params_opt_exclude = initialize_params(
        init_pt_cld, num_frames, mean3_sq_dist, device, load_semantics
    )
...
def transformed_semantics2rendervar(params, transformed_gaussians, device="cuda"):
    ...
    rendervar = {
        ...
        "means2D": torch.zeros_like(
            params["means3D"], requires_grad=True, device=device
        )
        + 0,
    }
    return rendervar
...
def get_loss(
    ...
    if load_semantics:
        ...
        (
            ...
            _,
            _,
        ) = Renderer(raster_settings=curr_data["cam"])(**semantic_rendervar)
    ...
    if tracking and (use_sil_for_loss or ignore_outlier_depth_loss):
        ...
        if load_semantics:
            losses["seg"] = torch.abs(curr_data["semantic_color"] - rendered_seg)[
                color_mask
            ].sum()
    elif tracking:
        ...
        if load_semantics:
            losses["seg"] = 0.8 * l1_loss_v1(
                rendered_seg, curr_data["semantic_color"]
...
def initialize_new_params(
    new_pt_cld, mean3_sq_dist, device, load_semantics=False, params_opt_exclude=None
):
...
def densify(params, variables, optimizer, iter, densify_dict, device="cuda"):
    if iter <= densify_dict["stop_after"]:
        ...
        if (iter >= densify_dict["start_after"]) and (
            ...
            params = cat_params_to_optimizer(
                new_params, params, params_opt_exclude, optimizer
            )
            num_pts = params["means3D"].shape[0]
            ...
            rots = build_rotation(
                params["unnorm_rotations"][to_split], device=device
            ).repeat(n, 1, 1)
            ...
            to_remove = torch.cat(
                (
                    to_split,
                    torch.zeros(n * to_split.sum(), dtype=torch.bool, device=device),
                )
            )
            params, variables = remove_points(
                to_remove, params, params_opt_exclude, variables, optimizer
