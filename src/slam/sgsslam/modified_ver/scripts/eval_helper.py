...
def recolor_semantic_img(rendered_seg, gt_seg, gt_id, color_map=None):
    ...
    refer_color = (
        color_map.reshape(1, -1, 3).type(torch.float64).to(gt_seg.device)
    )  # (1, H*W, 3)
...
def recolor_semantic_img_v2(rendered_seg, gt_seg, gt_id, color_map=None, n_class=102):
    ...
    refer_color = (
        color_map.reshape(1, -1, 3).type(torch.float32).to(gt_seg.device)
    )  # (1, H*W, 3)
    ...
    closest_indices = torch.argmin(l1_distances, axis=1)
    if color_map is None:
        rendered_logits = torch.zeros((img_shape[0] * img_shape[1], n_class)).to(
            l1_distances
        )
        rendered_logits[..., id_map.squeeze()] = F.softmax(
            -l1_distances.clone(), dim=-1
        )
    else:
    ...
    rendered_logits = rendered_logits.reshape(img_shape[0], img_shape[1], -1).permute(
        2, 0, 1
    )  # C, H,W
...
def report_loss(
    losses, wandb_run, wandb_step, tracking=False, mapping=False, load_semantics=False
):
    # Update loss dict
    loss_dict = {
...
def plot_rgbd_silhouette(
    ...
    if seg is not None:
        rastered_seg, render_id = recolor_semantic_img(
            rastered_seg, seg, seg_id, color_map
        )
...
def report_progress(
    ...
    if i % every_i == 0 or i == 1:
        ...
        depth_sil_rendervar = transformed_params2depthplussilhouette(
            params, data["w2c"], transformed_pts
        )
        (
            depth_sil,
            _,
        ...
        if load_semantics:
            semantic_rendervar = transformed_semantics2rendervar(
                params, transformed_pts, device=device
            )
            (
                rastered_seg,
                _,
                _,
            ) = Renderer(raster_settings=data["cam"])(**semantic_rendervar)
            ...
            miou = calc_miou_v2(
                pred_color=rastered_seg.permute(1, 2, 0),
                target_color=gt_seg,
                target_id=gt_id,
            )
        else:
...
def calc_miou_v2(
    pred_color: torch.Tensor, target_color: torch.Tensor, target_id: torch.Tensor
) -> float:
    """
...
def eval_semantic(
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        seman_gt_color = (
            apply_colormap(seman_gt_id[0].cpu().long().numpy(), sem_colormap) / 255.0
        )
        ...
        curr_data = {
            "cam": cam,
            "im": color,
            "seman_gt_id": seman_gt_id[0].long(),
            "seman_gt_color": seman_gt_color,
            "seman_pseudo_id": seman_pseudo_id.long(),
            "seman_pseudo_color": seman_pseudo_color,
            "id": time_idx,
        ...
        # Initialize Render Variables
        semantic_rendervar = transformed_semantics2rendervar(
            final_params, transformed_gaussians, device=slam_model.device
        )
        depth_sil_rendervar = transformed_params2depthplussilhouette(
        ...
        rastered_seg, _, depth = Renderer(raster_settings=curr_data["cam"])(
            **semantic_rendervar
        )
        rastered_seg = torch.clamp(rastered_seg, min=0, max=1)

        recolored_seg_curr = recolor_semantic_img(
            rastered_seg.clone(),
            curr_data["seman_gt_color"],
            curr_data["seman_gt_id"],
            color_map=None,
        )
        miou_g_curr = calc_miou_v2(
            pred_color=recolored_seg_curr.permute(1, 2, 0),
            target_color=curr_data["seman_gt_color"],
            target_id=curr_data["seman_gt_id"],
        )
        miou_g_curr_list.append(miou_g_curr)

        color_map = torch.from_numpy(sem_colormap.copy() / 255.0).to(
            rastered_seg.device
        )
        recolored_seg, rendered_id, rendered_logits = recolor_semantic_img_v2(
            rastered_seg.clone(),
            curr_data["seman_gt_color"],
            curr_data["seman_gt_id"],
            color_map.clone(),
        ...
        miou_g = calc_miou(
            pred=rendered_id.long(), target=curr_data["seman_gt_id"].long()
        )
        miou_p = calc_miou(
            pred=curr_data["seman_pseudo_id"].long(),
            target=curr_data["seman_gt_id"].long(),
        )
        ...
        topks_g = calc_topk_acc(
            pred_logits=rendered_logits.permute(1, 2, 0),
            target=curr_data["seman_gt_id"].long(),
            topk=(1, 3, 5),
        )
        mAP_g = calc_mAP(
        ...
        original_render_rgb = (
            torch.from_numpy(rastered_seman_rgb).permute(1, 2, 0).cpu().numpy() * 255
        )
        original_render_rgb = original_render_rgb.astype(np.uint8)
        cv2.imwrite(
            save_path, cv2.cvtColor(original_render_rgb, cv2.COLOR_RGB2BGR)
        )  # Convert RGB to BGR for OpenCV

        save_path = os.path.join(
            plot_dir, f"{plot_name}_recolor_curr_miou_{miou_g_curr:.4f}.png"
        )
        render_seman_rgb_curr = (
            torch.from_numpy(recolored_seman_rgb_curr).permute(1, 2, 0).cpu().numpy()
            * 255
        )
        render_seman_rgb_curr = render_seman_rgb_curr.astype(np.uint8)
        cv2.imwrite(
            save_path, cv2.cvtColor(render_seman_rgb_curr, cv2.COLOR_RGB2BGR)
        )  # Convert RGB to BGR for OpenCV

        save_path = os.path.join(
            plot_dir, f"{plot_name}_recolor_global_miou_{miou_g:.4f}.png"
        )
        render_seman_rgb_global = (
            torch.from_numpy(recolored_seman_rgb).permute(1, 2, 0).cpu().numpy() * 255
        )
        render_seman_rgb_global = render_seman_rgb_global.astype(np.uint8)
        cv2.imwrite(
            save_path, cv2.cvtColor(render_seman_rgb_global, cv2.COLOR_RGB2BGR)
        )  # Convert RGB to BGR for OpenCV
        ...
        pseudo_seman_rgb = torch.from_numpy(pseudo_seman_rgb).permute(
            2, 0, 1
        )  # 3, H ,W
        ...
        recolored_seman_curr = resize_tensor(
            recolored_seman_rgb_curr, *target_res, mode=mode
        )
        recolored_seman_np_curr = (
            recolored_seman_curr.permute(1, 2, 0).cpu().numpy() * 255
        )
