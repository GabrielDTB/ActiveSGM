...
from src.slam.semsplatam.modified_ver.semantic.oneformer import (
    oneformer_segmentation,
    positive_normalize,
)
...
class SemSplatam(SplatamOurs):
    def __init__(
        ...
        self.oneformer_processor.image_processor.num_text = (
            self.oneformer_model.config.num_queries
            - self.oneformer_model.config.text_encoder_n_ctx
        )
        ...
        if os.path.exists(class_info_file):
            ...
            info_semantic_file = os.path.join(
                self.slam_cfg["semantic_dir"], "info_semantic.json"
            )
        ...
        modify_metadata(
            class_info_file=self.class_info_file, processor=self.oneformer_processor
        )
    ...
    def semantic_annotation(self, input_img: torch.Tensor):
        ...
        class_ids_from_oneformer, class_logits_from_oneformer = oneformer_segmentation(
            input_img,
        ...
        return class_ids_from_oneformer[0], class_logits_from_oneformer[
            0
        ]  # (H,W), (H,W,150)
    ...
    def render(
        ...
        depth_sil_rendervar = transformed_params2depthplussilhouette(
            params, first_frame_w2c, transformed_gaussians
        )
        ...
        (
            ...
            _,
            _,
        ) = Renderer(raster_settings=cam)(**depth_sil_rendervar)
    ...
    def render_semantic(
        self, c2w: torch.Tensor, seen: torch.Tensor
        ...
        seman_rendervar = transformed_params2semrendervar_sparse(
            params, transformed_gaussians, seen
        )
        sparse_cam = set_camera_sparse(cam=cam, cls_ids=variables["seman_cls_ids"])
        (
            logits,
            _,
        ) = SEMRenderer_sparse(raster_settings=sparse_cam)(**seman_rendervar)
    ...
    def update_global_keyframe_set_completeness(
        self,
        depth,
        c2w,
        thre,
    ...
    def update_global_keyframe_set_quality(
        self,
        color,
        depth,
        c2w,
        ...
        if not (dont_add_kf):
            if (
                ...
                with torch.no_grad():
                    ...
                    color_ig = calc_psnr(
                        render_color * valid_depth_mask, color * valid_depth_mask
                    ).mean()
                    depth_ig = (
                        torch.abs(
                            render_depth * valid_depth_mask - depth * valid_depth_mask
                        )
                        / (depth + 1e-8)
    ...
    def update_global_keyframe_set_quality_rel(
        self,
        ...
        for kf in self.keyframe_list[:-5]:
            ...
            color_ig = calc_psnr(
                color * valid_depth_mask, kf["color"] * valid_depth_mask
            ).mean()
        ...
        if len(color_igs) > 0:
            ...
            ### Rank based on PSNR ###
            color_thre = torch.quantile(
                color_igs, self.slam_cfg.global_keyframe.quality_perc_thre / 100.0
            )
        ...
        if len(seman_igs) > 0:
            ...
            seman_thre = torch.quantile(
                seman_igs, self.slam_cfg.global_keyframe.quality_perc_thre / 100.0
            )
            seman_kf_idxs = torch.where(seman_igs <= seman_thre)[0]
            selected_kf_idxs = torch.unique(
                torch.cat((selected_kf_idxs, seman_kf_idxs))
            )
        ...
        if len(selected_kf_idxs) > 0:
            ...
            new_kf = [
                elem.item()
                for elem in selected_kf_idxs
                if elem not in self.global_keyframe_indices
            ]
    ...
    def update_explr_map(
        ...
        if (
            time_idx == 0
            or (time_idx + 1) % config["map_every"] == 0
            or force_map_update
        ):
    ...
    def update_gs_map(
        ...
        if seperate_tracking_res:
            ### Load tracking data ###
            tracking_h, tracking_w = (
                self.config["data"]["tracking_image_height"],
                self.config["data"]["tracking_image_width"],
            )
            tracking_color = F.interpolate(
                color.unsqueeze(0), (tracking_h, tracking_w), mode="bilinear"
            )[0]
            tracking_depth = F.interpolate(
                depth.unsqueeze(0), (tracking_h, tracking_w), mode="nearest"
            )[0]
        ...
        ) and not config["tracking"]["use_gt_poses"]:
            try:
        ...
        if (
            ...
            if config["mapping"]["add_new_gaussians"] and time_idx > 0:
                ...
                if seperate_densification_res:
                    ...
                    densify_h, densify_w = (
                        self.config["data"]["densification_image_height"],
                        self.config["data"]["densification_image_width"],
                    )
                    densify_color = F.interpolate(
                        color.unsqueeze(0), (densify_h, densify_w), mode="bilinear"
                    )[0]
                    densify_depth = F.interpolate(
                        depth.unsqueeze(0), (densify_h, densify_w), mode="nearest"
                    )[0]
                    densify_seman = F.interpolate(
                        seman.unsqueeze(0), (densify_h, densify_w), mode="bilinear"
                    )[0]
                    densify_curr_data = {
                        "cam": densify_cam,
                        "im": densify_color,
            ...
            with torch.no_grad():
                ...
                if PRINT_INFO:
                    ...
                    if self.slam_cfg.use_global_keyframe:
                        global_keyframe_time_indices = [
                            frame_idx
                            for frame_idx in self.global_keyframe_time_indices
            ...
            for iter in range(num_iters_mapping):
                ...
                if only_use_global_keyframe or (
                    self.slam_cfg.use_global_keyframe and iter > num_iters_mapping // 2
                ):
                    ### Global Keyframe ###
                    if len(self.global_keyframe_indices) == 1:
                        ...
                        selected_rand_keyframe_idx = np.random.choice(
                            self.global_keyframe_indices[:-1]
                        )
                        iter_time_idx = keyframe_list[selected_rand_keyframe_idx]["id"]
                        iter_color = keyframe_list[selected_rand_keyframe_idx][
                            "color"
                        ]  # C,H,W
                        iter_depth = keyframe_list[selected_rand_keyframe_idx]["depth"]
                        iter_seg_img = (
                            iter_color.permute(1, 2, 0).clone().to(self.semantic_device)
                        )
                        _, iter_seman = self.semantic_annotation(iter_seg_img)
                        iter_seman = iter_seman.permute(2, 0, 1).to(self.device)
                        if (
                            "crop_mask"
                            in keyframe_list[selected_rand_keyframe_idx].keys()
                        ):
                            iter_crop_mask = keyframe_list[selected_rand_keyframe_idx][
                                "crop_mask"
                            ]
                        else:
                            iter_crop_mask = get_uncert_mask(
                                iter_seman,
                                filter_pct=0.1,
                                thres=self.slam_cfg["uncert_mask_thres"],
                            )
                            keyframe_list[selected_rand_keyframe_idx]["crop_mask"] = (
                                iter_crop_mask
                            )
                else:
        ...
        ##################################################
        if self.slam_cfg.use_global_keyframe and not (only_use_global_keyframe):
            quality_method = self.slam_cfg.global_keyframe.get(
                "quality_method", "absolute"
            )
            if quality_method == "absolute":
                ...
                if (
                    time_idx > 0
                    and time_idx % self.slam_cfg.global_keyframe.quality_freq == 0
                ):
    ...
    def print_and_save_result(
        self, eval_dir_suffix="", is_prune_gaussians=False, ignore_first_frame=False
    ):
        ...
        num_frames = self.num_frames
        eval_dir = (
            self.eval_dir + "_" + eval_dir_suffix if eval_dir_suffix else self.eval_dir
        )
        ...
        if is_prune_gaussians:
            ...
            params, variables = prune_gaussians_w_semantic(
                params, variables, optimizer, 0, config["mapping"]["pruning_dict"]
            )
        ...
        if config["use_wandb"]:
            wandb_run.log(
                {
                    "Final Stats/Average Tracking Iteration Time (ms)": tracking_iter_time_avg
                    * 1000,
                    "Final Stats/Average Tracking Frame Time (s)": tracking_frame_time_avg,
                    "Final Stats/Average Mapping Iteration Time (ms)": mapping_iter_time_avg
                    * 1000,
        ...
        params["seman_cls_ids"] = (
            variables["seman_cls_ids"].detach().cpu().contiguous().numpy()
        )

        # Save Parameters
        results_dir = (
            os.path.join(self.results_dir, eval_dir_suffix)
            if eval_dir_suffix
    ...
    def load_params_by_step(self, step=1100, stage="final"):
        ...
        if checkpoint_time_idx == 0:
            ckpt_path = os.path.join(
                config["workdir"], config["run_name"], f"{stage}/params.npz"
            )
        else:
            ckpt_path = os.path.join(
                config["workdir"],
    ...
    def eval_result(
        self, eval_dir_suffix="", ignore_first_frame=False, save_frames=False
    ):
