...
def positive_normalize(logits, dim=-1, min=0.0):  # logits: [..., num_classes]
    mask = logits > min
    sum_positive = torch.sum(logits * mask, dim=dim, keepdim=True)
    safe_sum = torch.where(
        sum_positive == 0, torch.tensor(1.0, device=logits.device), sum_positive
    )
    normalized_logits = torch.where(
        mask, logits / safe_sum, torch.tensor(0.0, device=logits.device)
    )
    return normalized_logits


def oneformer_segmentation(
    image, oneformer_processor, oneformer_model, rank, num_classes=102
):
    inputs = oneformer_processor(
