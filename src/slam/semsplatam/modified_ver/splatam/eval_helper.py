...
def plot_rgbds_silhouette_fast(
    color,
    depth,
    seman_rgb,
    rastered_color,
    ...
    seman_rgb = torch.from_numpy(seman_rgb / 255.0).permute(2, 0, 1)  # 3, H ,W
    ...
    rastered_seman = torch.from_numpy(rastered_seman_rgb / 255.0).permute(2, 0, 1)
    ...
    depth_np = cv2.applyColorMap(
        cv2.convertScaleAbs(depth_np, alpha=255 / 6.0), cv2.COLORMAP_JET
    )
    rastered_depth_np = cv2.applyColorMap(
        cv2.convertScaleAbs(rastered_depth_np, alpha=255 / 6.0), cv2.COLORMAP_JET
    )
    diff_depth_l1_np = cv2.applyColorMap(
        cv2.convertScaleAbs(diff_depth_l1_np, alpha=255 / 6.0), cv2.COLORMAP_JET
    )
    diff_depth_l1_np = cv2.resize(
        diff_depth_l1_np, target_res, interpolation=cv2.INTER_NEAREST
    )

    # Convert Silhouette or diff_rgb to numpy, if applicable
    if diff_rgb is not None:
        diff_rgb_np = diff_rgb.cpu().numpy()  # (H, W)
        diff_rgb_np = cv2.applyColorMap(
            cv2.convertScaleAbs(diff_rgb_np, alpha=255 / 6.0), cv2.COLORMAP_JET
        )
    else:
        # Resize presence_sil_mask to match the target resolution
        presence_sil_mask_np = cv2.resize(
            presence_sil_mask.astype(np.uint8) * 255,
            target_res,
    ...
    row1 = np.hstack(
        [
            color_np,
            depth_np,
            diff_rgb_np if diff_rgb is not None else presence_sil_mask_np,
            seman_np,
        ]
    )
    row2 = np.hstack(
        [rastered_color_np, rastered_depth_np, diff_depth_l1_np, rastered_seman_np]
    ).astype(np.uint8)
    ...
    if save_plot and plot_dir is not None and plot_name is not None:
        ...
        cv2.imwrite(
            save_path, cv2.cvtColor(final_image, cv2.COLOR_RGB2BGR)
...
def eval(
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        depth_sil_rendervar = transformed_params2depthplussilhouette(
            final_params, curr_data["w2c"], transformed_gaussians
        )
        ...
        (
            ...
            _,
            _,
        ) = Renderer(raster_settings=curr_data["cam"])(**depth_sil_rendervar)
        ...
        seman_rendervar = transformed_params2semrendervar(
            final_params, final_variables, transformed_gaussians, seen
        )
        (
            rastered_seman,
            _,
        ) = SEMRenderer(raster_settings=curr_data["cam"])(**seman_rendervar)  # 133.H.W
        ...
        # Compute Depth RMSE
        if mapping_iters == 0 and not add_new_gaussians:
            diff_depth_rmse = torch.sqrt(
                (((rastered_depth - curr_data["depth"]) * presence_sil_mask) ** 2)
            )
            diff_depth_rmse = diff_depth_rmse * valid_depth_mask
            rmse = diff_depth_rmse.sum() / valid_depth_mask.sum()
            diff_depth_l1 = torch.abs(
                (rastered_depth - curr_data["depth"]) * presence_sil_mask
            )
        ...
        if save_frames:
            ...
            depth_colormap = cv2.applyColorMap(
                (normalized_depth * 255).astype(np.uint8), cv2.COLORMAP_JET
            )
            cv2.imwrite(
                os.path.join(render_rgb_dir, "gs_{:04d}.png".format(time_idx)),
            ...
            cv2.imwrite(
                os.path.join(rgb_dir, "gt_{:04d}.png".format(time_idx)),
                cv2.cvtColor(viz_gt_im * 255, cv2.COLOR_RGB2BGR),
            )
            cv2.imwrite(
                os.path.join(depth_dir, "gt_{:04d}.png".format(time_idx)),
                depth_colormap,
            )

            # Save Semantic RGB
            cv2.imwrite(
                os.path.join(seman_dir, "gt_{:04d}.png".format(time_idx)),
    ...
    if wandb_run is not None:
        wandb_run.log(
            {
                "Final Stats/Average PSNR": avg_psnr,
    ...
    fig.suptitle(
        "Average PSNR: {:.2f}, Average Depth L1: {:.2f} cm, ATE RMSE: {:.2f} cm".format(
            avg_psnr, avg_l1 * 100, ate_rmse * 100
        ),
        y=1.05,
        fontsize=16,
    )
    ...
    if i % every_i == 0 or i == 1:
        ...
        if tracking:
            ...
            if iter_time_idx > 0:
                ...
                rel_est_w2c = relative_transformation(
                    latest_est_w2c_list[-2], latest_est_w2c_list[-1]
                )
            ...
            if wandb_run is not None:
                tracking_log = {
                    f"{stage}/Latest Pose Error": iter_pt_error,
        ...
        depth_sil_rendervar = transformed_params2depthplussilhouette(
            params, data["w2c"], transformed_gaussians
        )
        (
            depth_sil,
            _,
            _,
        ) = Renderer(raster_settings=data["cam"])(**depth_sil_rendervar)
        rastered_depth = depth_sil[0, :, :].unsqueeze(0)
        ...
        seman_rendervar = transformed_params2semrendervar(
            params, variables, transformed_gaussians, seen
        )
        (
            rastered_seman,
            _,
        ) = SEMRenderer(raster_settings=data["cam"])(**seman_rendervar)  # 133.H.W
        ...
        if tracking:
            psnr = calc_psnr(
                im * presence_sil_mask, data["im"] * presence_sil_mask
            ).mean()
        else:
        ...
        if tracking:
            diff_depth_rmse = torch.sqrt(
                (((rastered_depth - data["depth"]) * presence_sil_mask) ** 2)
            )
            diff_depth_rmse = diff_depth_rmse * valid_depth_mask
            rmse = diff_depth_rmse.sum() / valid_depth_mask.sum()
            diff_depth_l1 = torch.abs(
                (rastered_depth - data["depth"]) * presence_sil_mask
            )
            diff_depth_l1 = diff_depth_l1 * valid_depth_mask
            depth_l1 = diff_depth_l1.sum() / valid_depth_mask.sum()
        else:
        ...
        if not (tracking or mapping):
            progress_bar.set_postfix(
                {
                    f"Time-Step: {iter_time_idx} | PSNR: {psnr:.{7}} | Depth RMSE: {rmse:.{7}} | L1": f"{depth_l1:.{7}}"
                }
            )
            ...
            progress_bar.set_postfix(
                {
                    f"Time-Step: {online_time_idx} | Frame {data['id']} | PSNR: {psnr:.{7}} | Depth RMSE: {rmse:.{7}} | L1": f"{depth_l1:.{7}}"
                }
            )
            progress_bar.update(every_i)
        ...
        if wandb_save_qual and (i % qual_every_i == 0 or i == 1):
            ...
            if not mapping:
                fig_title = (
                    f"Time-Step: {iter_time_idx} | Iter: {i} | Frame: {data['id']}"
                )
            else:
                fig_title = (
                    f"Time-Step: {online_time_idx} | Iter: {i} | Frame: {data['id']}"
                )
            plot_rgbds_silhouette_fast(
                data["im"],
                data["depth"],
...
def calc_miou(pred: torch.Tensor, target: torch.Tensor) -> float:
    ...
    for cls in classes:
        ...
        if union == 0:
            iou = torch.tensor(
                float("nan")
            )  # Class not present in prediction and ground truth
        else:
...
def calc_topk_acc(pred_logits: torch.Tensor, target: torch.Tensor, topk=(1,)):
    ...
    """
    assert pred_logits.shape[:2] == target.shape, (
        "Shape mismatch between logits and target"
    )
    ...
    res = []
    for maxk in topk:
        _, pred_topk = pred_logits_flat_valid.topk(
...
def calc_mAP(pred_logits, target):
    ...
    for cls in range(num_classes):
        ...
        precision = cum_true / (
            torch.arange(1, len(true_sorted) + 1, device=pred_logits.device)
        )

        total_positives = true_binary.sum()
        if total_positives == 0:
            ap = torch.tensor(float("nan")).to(
                pred_logits.device
            )  # Ignore if class not present in GT
        else:
...
def eval_semantic(
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        topks_g = calc_topk_acc(
            pred_logits=rastered_seman,
            target=curr_data["seman_gt"].long(),
            topk=(1, 3, 5),
        )
        topks_p = calc_topk_acc(
            pred_logits=curr_data["seman_pseudo_logits"],
            target=curr_data["seman_gt"].long(),
            topk=(1, 3, 5),
        )
        ...
        mAP_g = calc_mAP(
            pred_logits=rastered_seman, target=curr_data["seman_gt"].long()
        )
        mAP_p = calc_mAP(
            pred_logits=curr_data["seman_pseudo_logits"],
            target=curr_data["seman_gt"].long(),
        ...
        miou_p = calc_miou(
            pred=curr_data["seman_pseudo"].long(), target=curr_data["seman_gt"].long()
        )
        ...
        # recolor seg
        reprocess_ids = post_precess_seg(
            rastered_seman.clone(), curr_data["seman_gt"].long()
        )
        reprocess_pseudo_ids = post_precess_seg(
            curr_data["seman_pseudo_logits"].clone(), curr_data["seman_gt"].long()
        )
        miou_g_curr = calc_miou(pred=reprocess_ids, target=curr_data["seman_gt"].long())
        miou_p_curr = calc_miou(
            pred=reprocess_pseudo_ids, target=curr_data["seman_gt"].long()
        )
        ...
        cv2.imwrite(
            save_path, cv2.cvtColor(gt_seman_rgb, cv2.COLOR_RGB2BGR)
        )  # Convert RGB to BGR for OpenCV

        save_path = os.path.join(
            plot_dir, f"{plot_name}_render_miou_{miou_g_curr:.4f}.png"
        )
        cv2.imwrite(
            save_path, cv2.cvtColor(recolored_rastered_seman_rgb, cv2.COLOR_RGB2BGR)
        )  # Convert RGB to BGR for OpenCV
        ...
        target_res = (256, 256)
        mode = "nearest"
        gt_seman_rgb = torch.from_numpy(gt_seman_rgb / 255.0).permute(
            2, 0, 1
        )  # 3, H ,W
        gt_seman = resize_tensor(gt_seman_rgb, *target_res, mode=mode)
        gt_seman_np = gt_seman.permute(1, 2, 0).cpu().numpy() * 255

        pseudo_seman_rgb = torch.from_numpy(pseudo_seman_rgb / 255.0).permute(
            2, 0, 1
        )  # 3, H ,W
        pseudo_seman = resize_tensor(pseudo_seman_rgb, *target_res, mode=mode)
        pseudo_seman_np = pseudo_seman.permute(1, 2, 0).cpu().numpy() * 255

        recolored_pseudo_seman_rgb = torch.from_numpy(
            recolored_pseudo_seman_rgb / 255.0
        ).permute(2, 0, 1)  # 3, H ,W
        recolored_pseudo_seman = resize_tensor(
            recolored_pseudo_seman_rgb, *target_res, mode=mode
        )
        recolored_pseudo_seman_np = (
            recolored_pseudo_seman.permute(1, 2, 0).cpu().numpy() * 255
        )

        rastered_seman_rgb = torch.from_numpy(rastered_seman_rgb / 255.0).permute(
            2, 0, 1
        )  # 3, H ,W
        rastered_seman = resize_tensor(rastered_seman_rgb, *target_res, mode=mode)
        rastered_seman_np = rastered_seman.permute(1, 2, 0).cpu().numpy() * 255

        recolored_rastered_seman_rgb = torch.from_numpy(
            recolored_rastered_seman_rgb / 255.0
        ).permute(2, 0, 1)  # 3, H ,W
        recolored_rastered_seman = resize_tensor(
            recolored_rastered_seman_rgb, *target_res, mode=mode
        )
        recolored_rastered_seman_np = (
            recolored_rastered_seman.permute(1, 2, 0).cpu().numpy() * 255
        )
...
def eval_semantic_mp3d(
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        iou_per_class_g = calc_iou_per_classes(
            pred=rastered_cls_ids,
            target=curr_data["seman_gt"].long(),
            target_classes=target_classes,
        )
        iou_per_class_p = calc_iou_per_classes(
            pred=curr_data["seman_pseudo"].long(),
            target=curr_data["seman_gt"].long(),
            target_classes=target_classes,
        )
    ...
    for i in range(len(target_classes)):
        print(
            f"Ours: IoU for {target_classes_name[i]}: {avg_iou_g_per_class[i].item() * 100:.2f}"
        )

    for i in range(len(target_classes)):
        print(
            f"Pseudo: IoU for {target_classes_name[i]}: {avg_iou_p_per_class[i].item() * 100:.2f}"
        )
    ...
    with open(os.path.join(eval_dir, "IoU_per_classes.txt"), "w") as f:
        ...
        for i in range(len(target_classes)):
            lines.append(
                f"iou_g {target_classes_name[i]}: {avg_iou_g_per_class[i].item() * 100}\n"
            )

        for i in range(len(target_classes)):
            lines.append(
                f"iou_p {target_classes_name[i]}: {avg_iou_p_per_class[i].item() * 100}\n"
            )
