...
def get_pointcloud_with_seman(
    ...
    seman = torch.permute(seman, (1, 2, 0)).reshape(
        h * w, -1
    )  # (C, H, W) -> (H, W, C) -> (H * W, C)
...
def initialize_params_with_seman(
    init_pt_cld, num_frames, mean3_sq_dist, gaussian_distribution, TOPK=16
):
    ...
    seman_sparse = create_differentiable_sparse_tensor(
        params["semantic_logits"], topk_indices, dense_shape
    )
    ...
    for k, v in params.items():
        ...
        if k in ["semantic_logits"]:
            ...
            params[k] = torch.nn.Parameter(
                sparse_values.cuda().float().contiguous().requires_grad_(True)
            )
        else:
            params[k] = torch.nn.Parameter(
...
def initialize_first_timestep(
    ...
    params, variables = initialize_params_with_seman(
        init_pt_cld, num_frames, mean3_sq_dist, gaussian_distribution, TOPK
    )
...
def initialize_new_params_with_seman(
    new_pt_cld, mean3_sq_dist, gaussian_distribution, TOPK=16
):
    ...
    for k, v in params.items():
        ...
        if k in ["semantic_logits"]:
            sparse_values = seman_sparse.coo()[2].reshape(dense_shape[0], TOPK)
            params[k] = torch.nn.Parameter(
                sparse_values.float().contiguous().requires_grad_(True)
            )
        else:
            if not isinstance(v, torch.Tensor):
                v = torch.tensor(v)
...
def add_new_gaussians_with_seman(
    params,
    variables,
    curr_data,
    sil_thres,
...
def transformed_params2semrendervar(params, variables, transformed_gaussians, seen):
    ...
    row_index = (
        torch.arange(N).repeat_interleave(topk).to(variables["seman_cls_ids"].device)
    )
    seman_sparse = SparseTensor(
        row=row_index.view(-1),
    ...
    rendervar = {
        ...
        "means2D": torch.zeros_like(
            params["means3D"][seen], requires_grad=True, device="cuda"
        )
        + 0,
    }
    return rendervar
...
def transformed_params2semrendervar_sparse(params, transformed_gaussians, seen):
    ...
    rendervar = {
        ...
        "rotations": F.normalize(
            transformed_gaussians["unnorm_rotations"][seen]
        ).detach(),
...
def calc_hellinger_distance(pred_dist, target_dist, eps=1e-8):
    ...
    pred_dist = pred_dist.reshape(-1, h * w).permute(1, 0)
    target_dist = target_dist.reshape(-1, h * w).permute(1, 0)
    pred_dist = torch.clamp(pred_dist, min=0.0)
...
def get_loss_with_seman(
    ...
    (
        ...
        radius,
        _,
    ) = Renderer(raster_settings=curr_data["cam"])(**rgb_rendervar)
    seen = radius > 0
    variables["means2D"] = rgb_rendervar[
        "means2D"
    ...
    sparse_cam = set_camera_sparse(
        cam=curr_data["cam"], cls_ids=variables["seman_cls_ids"]
    )
    (
        seman_im,
        _,
    ...
    if tracking and (use_sil_for_loss or ignore_outlier_depth_loss):
        ...
        losses["seman"] = (
            1 - calc_cosine(curr_data["seman"], seman_im, dim=0)[seman_mask].sum()
        )
    elif tracking:
        ...
        if "crop_mask" in curr_data.keys():
            ...
            losses["seman"] = lambda_hel * calc_hellinger_distance(
                pred_dist=seman_im, target_dist=seman_pseudo
            ) + lambda_cosine * (1 - calc_cosine(seman_pseudo, seman_im, dim=0))
        else:
            losses["seman"] = lambda_hel * calc_hellinger_distance(
                pred_dist=seman_im, target_dist=curr_data["seman"]
    ...
    if tracking and visualize_tracking_loss:
        ...
        diff_depth = (
            torch.abs(weighted_render_depth - weighted_depth).mean(dim=0).detach().cpu()
        )
