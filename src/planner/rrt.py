...
def trilinear_interpolation(voxel_grid: np.ndarray, point: np.ndarray) -> np.ndarray:
    ...
    if x < 0 or x > H - 1 or y < 0 or y > W - 1 or z < 0 or z > D - 1:
...
def is_collision_free(
    ...
    points = np.linspace(
        pa, pb, num=int(np.ceil(np.linalg.norm(pb - pa) / (step_size / 5))) + 1
    )
...
class Node:
    def __init__(self, x: float, y: float, z: float, device="cuda"):
        ...
        self._device = device
        self._xyz = (
            torch.tensor([self.x, self.y, self.z])
            .reshape(1, 3)
            .to(self._device)
            .float()
        )
...
class RRT(PathPlanner):
    def __init__(
        ...
        self.max_iter = (
            ...
            else torch.prod(torch.tensor(vol_shape)).item()
        )  # approximate the maximum sample number by the volume size
        ...
        self.z_range = (
            [margin, min(vol_shape[2] - 1 - margin, maxz)]
            if z_levels is None
            else z_levels
        )
        ...
        self.points = (
            torch.cat([x.reshape(-1, 1), y.reshape(-1, 1), z.reshape(-1, 1)], dim=1)
            .to(self._device)
            .float()
        )
    ...
    def start_new_plan(
        self, start: np.ndarray, goal: np.ndarray, sdf_map: np.ndarray
    ) -> None:
    ...
    def generate_random_point(
        ...
        if full_range:
            ...
            x, y, z = (
                self.free_xs[rand_idx],
                self.free_ys[rand_idx],
                self.free_zs[rand_idx],
            )
        else:
    ...
    def find_nearest_node(self, point: np.ndarray) -> Node:
        ...
        distances = torch.norm(
            torch.from_numpy(point).reshape(1, 3).to(self._device) - self.nodes_tensor,
            dim=1,
            keepdim=True,
        )
    ...
    def extend_tree(self, full_range: bool = False) -> None:
        ...
        _, complete_free = is_collision_free(
    ...
    def get_reachable_mask(self) -> np.ndarray:
        ...
        for i in range(num_repeat):
            points2nodes = self.points[i * batch_size : (i + 1) * batch_size].reshape(
                -1, 1, 3
            ) - self.nodes_tensor.reshape(1, -1, 3)
        ...
        ### last batch ###
        i += 1
        points2nodes = self.points[(i) * batch_size :].reshape(
            -1, 1, 3
        ) - self.nodes_tensor.reshape(1, -1, 3)
        dist = torch.norm(points2nodes, dim=2)
        min_dist, min_dist_idx = torch.min(dist, dim=1)
    ...
    def update_eval(
        self, is_valid_planning: bool, time: float, path: List[Node]
    ) -> None:
