...
def voxel_grid_sampling(points, voxel_size):
    ...
    for voxel_key, point_indices in tqdm(voxel_dict.items()):
        ...
        closest_point = min(
            point_indices, key=lambda idx: np.linalg.norm(points[idx] - voxel_center)
        )
...
def evenly_sample_points(gt_pts, pred_pts, pred_colors, num_sample_pts, voxel_size):
    ...
    if len(candidate_indices) > num_sample_pts:
        chosen_indices = np.random.choice(
            candidate_indices, size=num_sample_pts, replace=False
        )
    else:
    ...
    return (
        gt_pts[chosen_indices],
        pred_pts[nearest_pred_indices],
        pred_colors[nearest_pred_indices],
    )
...
def color_pointcloud_by_values(
    pointcloud: trimesh.PointCloud,
    values: np.ndarray,
    output_file: str = "colored_pointcloud.ply",
):
    """
    ...
    colors = (colormap(norm_values)[:, :3] * 255).astype(
        np.uint8
    )  # RGB, scaled to 0-255
...
def save_filtered_pointcloud(
    pointcloud: trimesh.PointCloud,
    mask: np.ndarray,
    output_file: str = "filtered_pointcloud.ply",
):
    """
    ...
    if pointcloud.colors is not None:
        ...
        filtered_pointcloud = trimesh.PointCloud(
            filtered_vertices, colors=filtered_colors
        )
    else:
...
def argument_parsing() -> argparse.Namespace:
    ...
    """
    parser = argparse.ArgumentParser(description="Arguments to run NVS evaluation.")
    parser.add_argument("--ckpt", type=str, default=None, help="ckpt to load in")
    parser.add_argument("--gt_mesh", type=str, help="ground truth mesh file path")
    parser.add_argument(
        "--transform_traj", type=str, help="trajectory file for initial pose loading"
    )
...
def convert_gs_to_pcd(
    params: Dict, num_pt: int = 200000, ply_path: str = None
) -> np.ndarray:
...
def transform_point_cloud(
    transformation: torch.Tensor, point_cloud: np.ndarray
) -> np.ndarray:
    """
    ...
    homogeneous_points = np.ones(
        (num_points, 4)
    )  ### comment: Create a (N, 4) array with ones in the last column
    homogeneous_points[:, :3] = (
        point_cloud  ### comment: Assign the original 3D coordinates to the first three columns
    )
    ...
    transformed_points = (
        homogeneous_points @ transformation_np.T
    )  ### comment: Matrix multiplication (N, 4) x (4, 4)^T

    # Convert back from homogeneous to 3D by dropping the fourth column
    transformed_points = (
        transformed_points[:, :3] / transformed_points[:, 3, np.newaxis]
    )  ### comment: Normalize by the last coordinate if it's not 1
...
color_pointcloud_by_values(
    rec_pc_tri, pt_err["dist_acc"], os.path.join(args.result_dir, "pt_err_accuracy.ply")
)
color_pointcloud_by_values(
    gt_pc_tri,
    pt_err["dist_comp"],
    os.path.join(args.result_dir, "pt_err_completeness.ply"),
)
save_filtered_pointcloud(
    gt_pc_tri,
    pt_err["comp_mask"],
    os.path.join(args.result_dir, "pt_err_complete_ratio.ply"),
)
