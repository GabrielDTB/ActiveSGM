...
def summarize_experiments(
    base_dir, experiment_list, output_csv="experiment_summary_activeslam_mp3d.csv"
):
        ...
        for scene in scenes:
            for exp in experiment_list:
                for stage in stages:
                    ...
                    splatam_stage_path = os.path.join(
                        base_dir, scene, exp, "run_0", "splatam", stage
                    )
                    stage_abbr = stage.replace("eval_", "")
                    eval_3d_stage_path = os.path.join(
                        base_dir, scene, exp, "run_0", "eval_3d", stage_abbr
                    )
                    ...
                    eval_3d_results_path = os.path.join(
                        eval_3d_stage_path, "eval_3d_result.txt"
                    )
                    if os.path.exists(eval_3d_results_path):
                        acc, comp, comp_percent = read_eval_3d_results(
                            eval_3d_results_path
                        )
                    else:
