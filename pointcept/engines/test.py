        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            if (
                ...
                if "origin_coord" in data_dict:
                    ...
                    ##### save pred point clound
                    ## TODO: SAVE THE predict

                    output_coords = data_dict['origin_coord'].copy()
                    output_class = pred.copy()
                    save_pred_path = os.path.join(save_path, "pred")
                    os.makedirs(save_pred_path, exist_ok=True)
                    save_pred_ply = os.path.join(save_pred_path,f"{data_name}_pred_{self.num_classes}_classes.ply")
                    save_gt_ply = os.path.join(save_pred_path,f"{data_name}_gt_{self.num_classes}_classes.ply")
                    random.seed(42)  # Set seed for reproducibility
                    colormap = cm.get_cmap('gist_ncar', self.num_classes)
                    fixed_colormap = (colormap(np.linspace(0, 1, self.num_classes))[:, :3] * 255).astype(np.uint8)
                    ...
                    arr["r"], arr["g"], arr["b"] = output_pred_color[:, 0], output_pred_color[:, 1], output_pred_color[
                                                                                                     :, 2]
                    with open(save_pred_ply, "wb") as f:
                        f.write(header);
                        arr.tofile(f)

                    output_gt_coords = data_dict['pc_coord'].copy()
                    output_gt_class = data_dict['pc_segment'].copy()
                    output_gt_class[output_gt_class < 0] = 0
                    output_gt_class[output_gt_class > self.num_classes] = 0
                    output_gt_color = fixed_colormap[output_gt_class]

                    N = output_gt_coords.shape[0]
                    header = (
                        "ply\nformat binary_little_endian 1.0\n"
                        f"element vertex {N}\n"
                        "property float x\nproperty float y\nproperty float z\n"
                    ...
                    dtype = np.dtype([("x", "<f4"), ("y", "<f4"), ("z", "<f4"), ("r", "u1"), ("g", "u1"), ("b", "u1")])
                    arr = np.empty(N, dtype=dtype)
                    arr["x"], arr["y"], arr["z"] = output_gt_coords[:, 0], output_gt_coords[:, 1], output_gt_coords[:, 2]
                    arr["r"], arr["g"], arr["b"] = output_gt_color[:, 0], output_gt_color[:, 1], output_gt_color[
                                                                                                     :, 2]
                    with open(save_gt_ply, "wb") as f:
                        f.write(header);
                        arr.tofile(f)


                    del output_coords, output_pred_color, output_gt_coords, output_gt_class, output_gt_color
            ...
            print(f"Test: {data_name} [{idx + 1}/{len(self.test_loader)}]-{segment.size} \n")
            print(f"Batch {batch_time.val:.3f} ({batch_time.avg:.3f}) \n")
            print(f"Accuracy {acc:.4f} ({m_acc:.4f}) \n")
            print(f"mIoU {iou:.4f} ({m_iou:.4f}) \n")

            save_pred_path = os.path.join(save_path, "pred")
            os.makedirs(save_pred_path, exist_ok=True)
            save_pred_text = os.path.join(save_pred_path,f"{data_name}_pred_{self.num_classes}_classes.txt")
            with open(save_pred_text, "w") as f:
                f.write(f"Test: {data_name} [{idx + 1}/{len(self.test_loader)}]-{segment.size}\n")
                f.write(f"Batch {batch_time.val:.3f} ({batch_time.avg:.3f})\n")
                f.write(f"Accuracy {acc:.4f} ({m_acc:.4f})\n")
                f.write(f"mIoU {iou:.4f} ({m_iou:.4f})\n")
            f.close()
